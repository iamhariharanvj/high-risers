...
import 'package:highrisers/2.0/pages/shutterView.dart';
import 'package:highrisers/2.0/pages/windowView.dart';
import 'doorView.dart';
import 'package:image_picker/image_picker.dart';

import 'anyWoodNilaiView.dart';
import 'looverView.dart';
import 'myNilaiView.dart';


class looverOrder extends StatefulWidget {
  final users;
  final index;
  const looverOrder({Key? key,required this.users, this.index}) : super(key: key);

  @override
  _looverOrderState createState() => _looverOrderState();
}

class _looverOrderState extends State<looverOrder> {
  File? _image;
  ...
  storeImage(image) async
  {
    ...
    try {
      ...
      var snapshot = _storage.ref()
          .child("orders/$folder/" + "Shutter Order")
          .putFile(file);
                  ...
                  Padding(
                    ...
                    child: InkWell(
                      onTap: () async {
                        ...
                        map["loover Orderimage"] = _image!.path;
                        widget.users["loovers"][widget.index] = map;
                        storeImage(_image);
                        Navigator.of(context).push(
                            MaterialPageRoute(builder: (context) => looverView(
