...
import 'package:highrisers/2.0/pages/poojaView.dart';
...
class poojaOrder extends StatefulWidget {
  final users;
  final index;
  const poojaOrder({Key? key,required this.users, this.index}) : super(key: key);



  @override
  _poojaOrderState createState() => _poojaOrderState();
}

class _poojaOrderState extends State<poojaOrder> {
  File? _image;
  ...
  storeImage(image) async
  {
    ...
    try {
      ...
      var snapshot = _storage.ref()
          .child("orders/$folder/" + "Poojadoor Order")
          .putFile(file);
                  ...
                  Padding(
                    ...
                    child: InkWell(
                      onTap: () async {
                        ...
                        map["poojadoor Orderimage"] = _image!.path;
                        widget.users["poojadoors"][widget.index] = map;
                        storeImage(_image);
                        Navigator.of(context).push(
                            MaterialPageRoute(builder: (context) => poojaView(
