...
class doorOrder extends StatefulWidget {
  final users;
  final index;
  const doorOrder({Key? key,required this.users, this.index}) : super(key: key);

  @override
  _doorOrderState createState() => _doorOrderState();
}

class _doorOrderState extends State<doorOrder> {
  File? _image;
  ...
  storeImage(image) async
  {
    ...
    try {
      ...
      var snapshot = _storage.ref()
          .child("orders/$folder/" + "Door Order")
          .putFile(file);
                  ...
                  Padding(
                    ...
                    child: InkWell(
                      onTap: () async {
                        ...
                        map["door Orderimage"] = _image!.path;
